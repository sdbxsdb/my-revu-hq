...
export function parseTwilioError(error: any): { message: string; code?: string } {
  ...
  switch (errorCode) {
    ...
    case 21408:
      // Check if it's a region/permissions issue vs opt-out
      if (
        ...
        return {
          message:
            "SMS permissions are not enabled for this country/region. This may be due to pending registration approval or a delay in Twilio's system. Please contact support if this persists.",
          code: '21408',
        };
      }
      return {
    ...
    default:
      ...
      if (
        ...
        errorMessage.toLowerCase().includes('not a valid phone')
      ) {
        return {
      ...
      if (
        ...
        errorMessage.toLowerCase().includes('opt')
      ) {
        return {
      ...
      return {
        message:
          'Failed to send SMS. Please verify the phone number is correct and capable of receiving text messages. If the problem persists, contact support.',
        code: errorCode,
      };
  }
}
